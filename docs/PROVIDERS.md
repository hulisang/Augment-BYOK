...
- **tools schema**：使用 strict JSON schema（补齐 `additionalProperties=false`；保留原 schema 的 `required`，仅过滤掉不存在于 `properties` 的键；剥离 `propertyNames`/`patternProperties`/`if`/`then`/`else` 等 OpenAI 不支持的关键字）
